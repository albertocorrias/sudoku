...
def index(request):
    ...
    # }
    context = set_up_new_puzzle(Game.EASY)
    template = loader.get_template('sudoku_app/index.html')
    return HttpResponse(template.render(context, request))

def set_up_new_puzzle(diffiulty_level):
    game_objects = Game.objects.filter(difficulty = diffiulty_level)
    sel_idx = random.randint(0,game_objects.count()-1)
    puzzle = game_objects[sel_idx]
    
    diff_level_form = DifficultyLevelForm(initial = {'difficulty_level' : diffiulty_level})
    ret = {
        'puzzle_hints': puzzle.hints_board,
        'puzzle_solution': puzzle.solved_board,
        'diff_level_form' : diff_level_form
